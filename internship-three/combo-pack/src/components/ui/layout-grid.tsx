        ...
        <div key={i} className={cn(card.className, "")}>
          <motion.div
            ...
            className={cn(
              ...
              selected?.id === card.id
                ? "rounded-lg cursor-pointer absolute inset-0 h-1/2 w-full md:w-1/2 m-auto z-50 flex justify-center items-center flex-wrap flex-col shadow-2xl"
                : lastSelected?.id === card.id
                ? "z-40 bg-white rounded-xl h-full w-full shadow-lg"
                : "bg-white rounded-xl h-full w-full shadow-lg hover:shadow-2xl"
            )}
            layout
...
const BlurImage = ({ card }: { card: Card }) => {
  ...
  return (
    <div className="relative w-full h-[15rem] overflow-hidden">
      ...
      >
        <div className="p-4 text-center text-white">{card.content}</div>
      </motion.div>
    </div>
  );
};
