...
export default function Hero({ darkMode }: { darkMode: boolean }) {
  return (
    <section className="flex justify-center h-[32rem] px-4 md:px-0">
      <Grid />
      <div className="flex flex-col gap-5 absolute items-center py-10">
        <div className="flex flex-col items-center md:items-start gap-3">
          <h1 className={`font-semibold text-3xl md:text-5xl text-center w-full ${darkMode ? 'text-white' : 'text-black'}`}>
            Zero to Hero
          </h1>
          <h1 className="text-purple-600 text-4xl md:text-7xl font-semibold">Data Engineering</h1>
          <Typewriter />
        </div>
        <p className={`text-center text-[0.65rem] md:text-base ${darkMode ? 'text-neutral-300' : 'text-[#8A90A2]'}`}>
